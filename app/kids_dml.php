	...
	$data = [
		...
		'photo' => Request::fileUpload('photo', [
			...
			'success' => function($name, $selected_id) {
				createThumbnail($name, getThumbnailSpecs('kids', 'photo', 'tv'));
				createThumbnail($name, getThumbnailSpecs('kids', 'photo', 'dv'));
			},
...
function kids_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		if(!check_record_permission('kids', $selected_id, 'view'))
			return $Translation['tableAccessDenied'];

		// can edit?
		$AllowUpdate = check_record_permission('kids', $selected_id, 'edit');

		// can delete?
		$AllowDelete = check_record_permission('kids', $selected_id, 'delete');
	...
	$templateCode = str_replace('<%%UPLOADFILE(photo)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="photo" id="photo" data-filetypes="jpg|jpeg|gif|png|webp" data-maxsize="2048000" accept=".jpg,.jpeg,.gif,.png,.webp">'), $templateCode);
