...
function kids_insert(&$error_message = '') {
	global $Translation;
	...
	$data = [
		...
		'pocket_money' => Request::val('pocket_money', ''),
		'photo' => Request::fileUpload('photo', [
			...
			'success' => function($name, $selected_id) {
				Thumbnail::create($name, getThumbnailSpecs('kids', 'photo', 'tv'));
				Thumbnail::create($name, getThumbnailSpecs('kids', 'photo', 'dv'));
			},
			'failure' => function($selected_id, $fileRemoved) {
				if(!strlen(Request::val('SelectedID'))) return '';

				/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
				return existing_value('kids', 'photo', Request::val('SelectedID'));
			},
		]),
	];
	...
	if(function_exists('kids_before_insert')) {
		$args = [];
		if(!kids_before_insert($data, getMemberInfo(), $args)) {
	...
	update_calc_fields('kids', $recID, calculated_fields()['kids']);
	...
	set_record_owner('kids', $recID, $recordOwner);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) kids_copy_children($recID, Request::val('SelectedID'));
...
function kids_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('kids', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `transactions` WHERE `kid`='" . makeSafe($id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'transactions', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'kids_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'kids_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
...
function kids_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('kids', $selected_id, 'edit')) return false;
	...
	$data = [
		...
		'photo' => Request::fileUpload('photo', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('kids', 'photo', $selected_id);
			},
		]),
	];
	...
	// get existing values
	$old_data = getRecord('kids', $selected_id);
	...
	if(function_exists('kids_before_update')) {
		...
		if(!kids_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('kids', $data['selectedID'], calculated_fields()['kids']);
	...
	set_record_owner('kids', $selected_id);
}

function kids_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('kids', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('kids', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('kids', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		$res = sql("SELECT * FROM `kids` WHERE `id`='" . makeSafe($selectedId) . "'", $eo);
	...
	// process buttons
	if($showSaveNew) {
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return kids_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
	...
	if($hasSelectedId) {
		$jdata = get_joined_record('kids', $selectedId);
		if($jdata === false) $jdata = get_defaults('kids');
		$rdata = $row;
	}
	$templateCode .= loadView('kids-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: kids_dv
	if(function_exists('kids_dv')) {
		$args = [];
		kids_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
