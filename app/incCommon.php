	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = array(
			...
			'kids' => array('Kids', '', 'resources/table_icons/kids.png', 'None'),
			'transactions' => array('Transactions', '', 'resources/table_icons/table_money.png', 'None')
		);
	...
	function get_sql_fields($table_name) {
		$sql_fields = array(
			...
			'transactions' => "`transactions`.`id` as 'id', IF(    CHAR_LENGTH(`kids1`.`name`), CONCAT_WS('',   `kids1`.`name`), '') as 'kid', if(`transactions`.`date`,date_format(`transactions`.`date`,'%d/%m/%Y'),'') as 'date', `transactions`.`amount` as 'amount', `transactions`.`description` as 'description', `transactions`.`balance` as 'balance'",
		);
