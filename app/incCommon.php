	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'kids' => ['Kids', '', 'resources/table_icons/kids.png', 'None'],
			'transactions' => ['Transactions', '', 'resources/table_icons/table_money.png', 'None'],
		];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'kids' => "`kids`.`id` as 'id', `kids`.`name` as 'name', `kids`.`pocket_money` as 'pocket_money', `kids`.`photo` as 'photo', `kids`.`last_balance` as 'last_balance'",
			'transactions' => "`transactions`.`id` as 'id', IF(    CHAR_LENGTH(`kids1`.`name`), CONCAT_WS('',   `kids1`.`name`), '') as 'kid', if(`transactions`.`date`,date_format(`transactions`.`date`,'%d/%m/%Y %h:%i %p'),'') as 'date', `transactions`.`amount` as 'amount', `transactions`.`description` as 'description', `transactions`.`balance` as 'balance'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'kids' => "`kids` ",
			'transactions' => "`transactions` LEFT JOIN `kids` as kids1 ON `kids1`.`id`=`transactions`.`kid` ",
		];
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> pocket money</a>
			</div>
	...
	function NavMenus($options = []) {
		...
		if(is_array($arrTables)) {
			foreach($arrTables as $tn => $tc) {
				...
				$tChkHL = array_search($tn, ['transactions']);
