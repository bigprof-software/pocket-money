<?php
	...
	*/
		...
		$all_tables = [
			/* ['table_name' => [table props assoc array] */   
				'kids' => [
					'Caption' => 'Kids',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/kids.png',
					'group' => $tg[0],
					'homepageShowCount' => 1
				],
				'transactions' => [
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		if($tableName=='kids' && $fieldName=='photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='kids' && $fieldName=='photo' && $view=='dv')
			...
			$schema = [
				...
				'transactions' => [
					...
					'amount' => [
						...
						'info' => [
							'caption' => 'Amount',
							'description' => 'Enter a postive value for deposits, or a negative value for withdrawals/spending.',
						],
					],
					'description' => [
						'appgini' => "TEXT NULL",
	...
	*/
	...
	function calculated_fields() {
		...
		return [
			'kids' => [
				'last_balance' => 'SELECT `transactions`.`balance` FROM `kids` 
					LEFT JOIN `transactions` ON `transactions`.`kid`=`kids`.`id` 
					WHERE `kids`.`id`=\'%ID%\'
					ORDER BY `transactions`.`date` DESC, `transactions`.`id` DESC
					LIMIT 1',
			],
			'transactions' => [
				'balance' => 'SELECT SUM(`transactions`.`amount`) FROM `transactions` 
					WHERE
					    (
					        `transactions`.`date` < (SELECT `transactions`.`date` FROM `transactions` WHERE `transactions`.`id` = \'%ID%\')
					        OR
					        (
					    ...
					    AND
					    `transactions`.`kid` = (SELECT `transactions`.`kid` FROM `transactions` WHERE `transactions`.`id` = \'%ID%\')',
			],
		];
	}
	#########################################################
